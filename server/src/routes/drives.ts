...
// ── DELETE /api/drives/:id — Admin deletes a drive ─────────────────────────
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!drive) throw new BadRequestError('Drive not found');
...
// ── PATCH /api/drives/:id — Admin updates drive status/title ───────────────
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (req.body.fundingGoal !== undefined) updates.fundingGoal = req.body.fundingGoal;
      if (req.body.date) updates.date = req.body.date;
      if (req.body.requiredRoles) {
        updates.requiredRoles = req.body.requiredRoles;
        updates.maxVolunteers = req.body.requiredRoles.reduce((acc: number, r: any) => acc + Number(r.capacity), 0);
      }
      const drive = await Drive.findByIdAndUpdate(driveId, updates, { new: true });
      if (!drive) throw new BadRequestError('Drive not found');
      res.json({ drive });
    } catch (error) {
      next(error);
